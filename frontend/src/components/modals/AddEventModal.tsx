...
import { X, Calendar as CalendarIcon, Clock, MapPin, UploadCloud, Tag, AlertCircle } from 'lucide-react';

interface AddEventModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (newEvent: Omit<Event, 'id' | 'status'>) => Promise<void>;
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave({
        ...
        startTime,
        endTime,
        location: location || 'VBS Main Campus',
        notes,
      });
      // Reset form
      setTitle('');
      ...
      const msg = (err as { message?: string })?.message || 'Failed to save event';
      setFormError(msg);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="e.g. Elocution Competition"
                  className={`w-full px-4 py-2.5 text-sm bg-[#faf8f5] border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#c85a28]/30 focus:border-[#c85a28] text-[#2d1f19] placeholder-[#b5a79f] ${
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-[#e2d7c9] hover:border-[#c85a28] rounded-2xl bg-[#faf8f5] hover:bg-[#fefaf6] cursor-pointer transition-colors text-center">
                ...
                <span className="text-[10px] text-[#a3948e] mt-0.5">PNG, JPG (Max 5MB)</span>
                <input
                  type="file"
                  accept="image/*"
